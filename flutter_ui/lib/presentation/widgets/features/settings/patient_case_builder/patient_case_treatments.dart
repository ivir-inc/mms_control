...
class PatientCaseTreatments extends StatefulWidget {
  // editMode: true → toggle treatments on/off in the patient case builder.
  //           false → show only enabled treatments; tapping opens body location.
  ...
  @override
  State<PatientCaseTreatments> createState() => _PatientCaseTreatmentsState();
}

class _PatientCaseTreatmentsState extends State<PatientCaseTreatments> {
  ...
  void initState() {
    ...
    if (widget.layoutEditMode) {
      // Request focus after the first frame so we become primary focus inside
      // MmsDismissibleScreen, allowing onKeyEvent to intercept Escape before
      // it reaches KeyboardListener.
  ...
  Widget _buildContent(
    ...
    final metadataMap = <String, PhysicalTreatmentTypeItem>{
      for (var item in metadata.physicalTreatmentType ?? [])
    ...
    if (!widget.editMode && !widget.layoutEditMode) {
      enabledNames =
          (patientCase!.treatments?.getEnabledTypes() ?? <String>[]).toSet();
    }
    ...
    var categories = _mergeWithMetadata(
        layoutState.config!.treatments, metadataMap.keys.toSet());
            ...
            child: Container(
              ...
              child: LayoutBuilder(
                builder: (context, inner) {
                  ...
                  // Count visible items per category so each row of three can
                  // pad to the same height as its tallest peer.
                  final counts = categories
                      .map((c) => c.allItems
                          .map((n) => metadataMap[n])
                          .whereType<PhysicalTreatmentTypeItem>()
                          .length)
                      .toList();
                  return Wrap(
                    spacing: 20,
                    ...
                    children: categories.asMap().entries.map((entry) {
                      ...
                      final rowMax = counts
                          ...
                          .fold(0, (a, b) => a > b ? a : b);
                      // Round up to even so the 2-column grid fills complete rows.
                      final minCells =
  ...
  Widget _buildCategoryBlock(
    ...
    Map<String, PhysicalTreatmentTypeItem> metadataMap,
    PatientCase? patientCase,
    Set<String>? enabledNames, {
    int minCells = 2,
  }) {
    ...
    if (widget.layoutEditMode) {
      ...
      // The dragged item is excluded from the grid so items reflow around the
      // gap naturally. Its LongPressDraggable is kept alive in an Offstage
      // widget (same GlobalKey → Flutter reuses the element) so onDragEnd
      // fires regardless of where the pointer is released.
      ...
      final draggedItem = isDraggingFromHere
          ...
          : null;
      // Original index of the dragged item in allItems — used to pick
      // insert-before vs insert-after when computing dropIndex per cell.
      ...
      final showGap = isDraggingFromHere &&
          ...
          _hoverDropIndex != null;
      List<PhysicalTreatmentTypeItem?> gridItems;
      if (showGap) {
        final gapPos = _hoverDropIndex!.clamp(0, allItemsWithoutDragged.length);
        gridItems = [
          ...allItemsWithoutDragged
              .sublist(0, gapPos)
              .cast<PhysicalTreatmentTypeItem?>(),
          null,
          ...allItemsWithoutDragged
              .sublist(gapPos)
              .cast<PhysicalTreatmentTypeItem?>(),
        ...
        gridItems =
            allItemsWithoutDragged.cast<PhysicalTreatmentTypeItem?>().toList();
      }
            ...
            child: LayoutBuilder(
              builder: (context, constraints) {
                ...
                return Column(
                  ...
                  children: [
                    // Dragged item lives here (invisible) so its
                    // LongPressDraggable state is preserved mid-drag.
                    if (draggedItem != null)
                      Offstage(
                        offstage: true,
                        child: KeyedSubtree(
                    ...
                    GridView.count(
                      ...
                      children: [
                        ...gridItems.map((item) {
                          ...
                          // When dragging an item forward (to a later list
                          // position), insert after the hovered cell so the
                          // gap appears to its right rather than its left.
  ...
  void _commitRename(BuildContext context, int index) {
    ...
    if (newName.isNotEmpty) {
      context
          ...
          .add(RenameTreatmentCategory(index, newName));
    }
    setState(() => _editingCategoryIndex = null);
  }

  Widget _draggableGridCell({
    required PhysicalTreatmentTypeItem item,
  ...
  // Shown in place of the dragged item's original slot while hovering within
  // the same category. Also a DragTarget so releasing over the gap works.
  ...
  Widget _actionButton(
      PhysicalTreatmentTypeItem treatment, PatientCase? patientCase) {
    final isActive =
        patientCase?.treatments?.isEnabled(treatment.name!) ?? false;
        ...
        if (!widget.editMode) {
          Navigator.of(context).push(MaterialPageRoute(
              builder: (context) => MmsDismissibleScreen(
                    const ValueKey("PatientCaseBodyLocation"),
                    PatientCaseBodyLocation(
                      editMode: false,
                      treatmentSelected: treatment.enumConstant,
          ...
          context.read<PatientCaseBuilderBloc>().add(
                PatientCaseBuilderEvent(
                  isActive
                      ? PatientCaseBuilderAction.disableTreatment
                      : PatientCaseBuilderAction.enableTreatment,
                  dynamicTreatmentName: treatment.name,
                ),
              );
        }
      },
  ...
  Widget _wrapWithChrome(Widget child, PatientCaseBuilderState state) {
    final caption = widget.layoutEditMode
        ? "Edit Treatment Layout"
        : "Edit Treatments - ${state.patientCaseUnderEditOrDisplay} "
  ...
  // Appends any metadata items not yet assigned to any layout slot into the
  // first category, so they remain visible and draggable rather than vanishing.
